...
class FullProductInfo extends Component {
    ...
    render() {
        ...
        const productSize = Object.entries(this.props.size).map((sizKey, i) => {
            if (sizKey[1] === 0) {
                return (
                    <button className='ShoeNotAvailableSize' disabled key={i}>
                        {sizKey[0]}
                    </button>
                );
            } else {
                ...
                return (
                    <button
                        className={checkIfClicked}
                        onClick={() => {
                            this.selectSizeHandler(sizKey);
                        }}
                        key={i}>
                        {sizKey[0]}
                    </button>
        ...
        let addToCartBtn = <Button clicked={() => this.addToCartHandler(selectedShoeId, selectedShoePrice, selectedShoeImg)}>Add to Cart</Button>;
        return (
            <React.Fragment>
                ...
                <div className='FullProductCard'>
                    <div className='FullProductLeft'>
                        ...
                        {this.state.madeOfClicked ? <ProductMadeOf class={'ShoeMaterials'} upper={this.props.madeOf.upper} lining={this.props.madeOf.lining} outerSole={this.props.madeOf.outerSole} /> : null}
                    </div>
                    <div className='FullProductRigth'>
                        ...
                        <Button btnType='madeOf' clicked={this.showMadeOf}>
                            Made off:
                        </Button>
